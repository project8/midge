...
namespace midge
{
    ...
    bool cluster_controller::start_controller_pre()
    {
        f_stream = new TFile( f_candidate_file.c_str(), "RECREATE" );
        f_threshold_tree = new TTree( get_name().c_str(), get_name().c_str() );
        f_threshold_tree->SetDirectory( f_stream );
        f_threshold_tree->Branch( "time", &f_time );
        f_threshold_tree->Branch( "frequency", &f_frequency );
        f_threshold_tree->Branch( "value", &f_value );

        return true;
    }

    bool cluster_controller::start_controller_post()
    {
        if( f_plot == true )
        {
        ...
        f_frequency_size = in< 1 >()->get_size();
        f_frequency_interval = in< 1 >()->get_interval();
        f_power = in< 1 >()->raw();

        f_background = new real_t[ f_frequency_size ];
        ...
        f_minimum_frequency_index = (count_t) (round( f_minimum_frequency / f_frequency_interval ));
        f_maximum_frequency_index = (count_t) (round( f_maximum_frequency / f_frequency_interval ));
        ...
        if( t_size != f_frequency_size )
        {
            throw error() << "cannot analyze background tree with size <" << t_size << "> against input with size <" << f_frequency_size << ">";
        }
    ...
    bool cluster_controller::execute_controller_pre()
    {
        return true;
    }

    bool cluster_controller::execute_controller_post()
    {
        ...
        for( count_t t_frequency_index = f_minimum_frequency_index; t_frequency_index <= f_maximum_frequency_index; t_frequency_index++ )
        {
            ...
            {
                f_value = t_value;
                f_frequency = f_frequency_interval * t_frequency_index;
                f_threshold_tree->Fill();
            }
        }

        out< 0 >()->set_stride( f_stride );

        return true;
    }

    bool cluster_controller::stop_controller_pre()
    {
        f_stream->cd();
    ...
    bool cluster_controller::stop_controller_post()
    {
        if( f_plot == true )
        {
            ...
            t_times.count() = (f_maximum_time_index - f_minimum_time_index) / f_stride;
            ...
            t_frequencies.low() = f_minimum_frequency_index * f_frequency_interval;
            t_frequencies.high() = f_maximum_frequency_index * f_frequency_interval;
